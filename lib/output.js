...
function withMetadata (withMetadata) {
  ...
  if (is.object(withMetadata)) {
    ...
    if (is.defined(withMetadata.profile)) {
      if (is.file(withMetadata.profile)) {
        this.options.withMetadataProfile = withMetadata.profile;
      } else {
        throw new Error('Invalid profile specified ' + withMetadata.profile);
      }
    }
  }
  return this;
